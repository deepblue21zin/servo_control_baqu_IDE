...
이 문서는 `servo_control_baqu` 문서 묶음의 현재 기준점을 정리한다. 최신 기준은 2026-03-24 코드와 bench 관찰값이다.
...
- `main.c` CSV와 keyboard snapshot은 `enc_cnt`, `enc_raw`, `req_hz`, `applied_hz`, `RUN`, `REV`, `command lifecycle`을 함께 남긴다.
...
4. [`change_code/2026-03-24.md`](change_code/2026-03-24.md)
...
현재 상태를 설명해야 할 때는 `README -> command lifecycle spec -> steering_portal -> change_code/2026-03-24.md` 순서로 보는 것이 가장 빠르다.
