...
8. [관련 파일 목록](#관련-파일-목록)
9. [TODO: 인지파트 확인 필요 사항](#todo-인지파트-확인-필요-사항)
...
│  [인지 모듈 PC]          [이더넷 허브/스위치]               │
│   10.177.21.1    ─────→  │                                  │
│   UDP 브로드캐스트        ├──────→ [조향 STM32] 10.177.21.100  │
│   포트 7000               ├──────→ [구동 모듈]                │
│                           └──────→ [기타 파트]                │
└─────────────────────────────────────────────────────────────┘
...
조향 STM32     10.177.21.100  UDP 패킷 수신자 (우리 보드)
...
**설정 파일:** `LWIP/App/lwip.c`

```c
IP_ADDRESS[0] = 10;   IP_ADDRESS[1] = 177;
IP_ADDRESS[2] = 21;   IP_ADDRESS[3] = 100;   // STM32 = 10.177.21.100
...
static void udp_recv_cb(...) {
    ...
    if (pkt.steering_angle < -90.0f || pkt.steering_angle > 90.0f) return;
...
아래 항목은 인지파트 담당자와 협의하여 코드에 반영해야 합니다.

### 1. UDP 포트 번호
```c
// ethernet_communication.h 14번째 줄
#define AUTODRIVE_UDP_PORT    7000   // ← 인지파트 확인 후 수정
```
...
- 유니캐스트 (10.177.21.100) → STM32에만 전송
- 멀티캐스트 → 특정 그룹에만 전송
